...
const FAKE_USER_ID = 40; // Use your own user ID from reservation state!
  ...
  methods: {
    async handleSeatConfirmed(id, ...allArgs){
      ...
      try {
        ...
        //stop the confirmation child component from displayiung 
        this.selectedSeat = {};
        
        //Make sure the seating diagram shows the booked seat as an orange seat belonging to the current user
        //(imdediately, not after a page reolad)
        ...
        console.log('Error posting seat confirm', err);
        this.err = err;
      }
    }, // handleSeatConfirmed()
    selectSeat(row, col){
